...
class RoboticArm:
    def init(self, webots_node, properties):
        ...
        self.__reset_joint_limits_push = {
            'joint1': (-1.0, 1.0),      # Limit rotation to be mostly forward -0.2, 0.2
            'joint2': (1.1, 1.1),       # Keep arm low 1.1
            'joint3': (-0.6, -0.4),     # Adjust for forward reach
            'joint4': (0.3, 0.6),
            'joint5': (1.4, 1.6),       # Keep wrist pointing down 1.4 1.6
            'joint6': (-0.2, 0.2),
        }
        ...
        self.resetworld = 0 # Counter for world reloads
    ...
    def random_box_pose(self):
        ...
        distance = np.random.uniform(0.0, 0.05)
        ...
        roll = np.random.uniform(0, 2 * math.pi)
        pitch = np.random.uniform(0, 2 * math.pi)
    ...
    def random_box_target(self):
        # This function will loop until a valid position is found that is outside the forbidden zone.
        # Get the gripper's current position as the center point
        center_pose = self.current_box_pose.copy()
        center_pose[3:] =  [0,0,0,1]
       
        #distance = np.random.uniform(0.12, 0.2) # 0.65, 0.2
        distance = np.random.uniform(0.31, 0.38)
        phi = np.random.uniform(-math.pi/1.7, math.pi/1.7)
       
        offset_x = distance  * math.cos(phi)
        offset_y = distance  * math.sin(phi)
        
        x = center_pose[0] + offset_x
        y = center_pose[1] + offset_y
        ...
        if msg.reset and not self.__reset_in_progress:
            ...
            if needs_hard_reset:
                ...
                self.__target_positions = [
                    ...
                    for (low, high) in self.__reset_joint_limits_push.values() # Changed to __reset_joint_limits
                ]
                self.__target_positions = np.zeros(6) if np.random.rand() < 0.1 else self.__target_positions
    ...
    def step(self):
        ...
        if self.__reset_in_progress_world:
            ...
            for i in range(len(self.__joint_motors)):
                ...
                if abs(target_pos - current_pos) > tolerance:
                    all_reached = False
            ...
            if not all_reached or self.__target_reset_attemp > 5:
                self.__target_reset_attemp = 0
                self.__reset_in_progress = True
                self.__reset_in_progress_world = False # CRITICAL: Exit this state
            return  # Exit immediately after starting the reload
        elif self.__reset_in_progress:
            if self.__target_reset_attemp > 15:
                self.__target_reset_attemp = 0
                self.__reset_in_progress_world = True # Transition to the world reload state
                if self.resetworld > 3: 
                    self.resetworld = 0
                    self.__robot.worldReload()
                self.resetworld += 1
                return # Exit immediately after starting the reload
            # Move arm towards target reset position
            for i in range(len(self.__joint_motors)):
                current_pos = self.__joint_sensors[i].getValue()
                ...
                if abs(target_pos - current_pos) > tolerance:
                    ...
                    self.__joint_motors[i].setVelocity(0.0)
            self.__target_reset_attemp += 1
            # If arm successfully reaches pose (and we didn't need a world reload)
            if all_reached:
                # Randomize box (this is a "soft" reset without a world reload)
                box_pose = self.random_box_pose()
                self.target_random_box = self.random_box_target()
                self.box_node.getField("translation").setSFVec3f(list(box_pose[:3]))
                self.box_node.getField("rotation").setSFRotation(quaternion_to_axis_angle(box_pose[3:]))
               
                # --- Finalize and Exit Reset State ---
                reset_done_msg = Bool()
                ...
                self.__target_reset_attemp = 0
                self.last_sync_timestamp = -1
                self.__reset_in_progress = False # CRITICAL: Exit this state
        else:
            self.__frames_elapsed += 1
            
            # Your normal operation logic (stuck detection, etc.) goes here.
            ...
            if np.all(np.abs(box_pos[:2]) > 0.9):
                self.box_node.getField("translation").setSFVec3f(list(self.current_box_pose[:3]))
                self.box_node.getField("rotation").setSFRotation(quaternion_to_axis_angle(self.current_box_pose[3:]))
