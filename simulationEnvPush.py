...
class SimulationEnvPush(BaseEnv):
    ...
    def compute_reward( self, achieved_goal: np.ndarray, desired_goal: np.ndarray, info: dict) -> float:
        ...
        reward = np.where(distance_to_target < 0.2, 10.0, 0.0)  # Reward is positive if close to target, negative otherwise
        reward += -5*distance_to_target - 0.5*distance_to_box - 0.5*height_diff
        reward = np.clip(reward, -5.0, 10.0)  # Clip the reward to a reasonable range
        if reward_for_approaching > 0:
            reward += 10*reward_for_approaching  # Encourage moving towards the target
            reward = np.clip(reward, -5.0, 50.0)  # Clip the reward to a reasonable range
    ...
    def _is_success(self, achieved_goal: np.ndarray, desired_goal: np.ndarray) -> bool:
        ...
        return distance < 0.2 # first try 0.2, then 0.05
...
if __name__ == '__main__':    
    ...
    env = SimulationEnvPush(port='1244', env_id=10)
    ...
    for episode in range(num_episodes):
        ...
        while not done:#and step_count < max_steps_per_episode :
            ...
            action, _states = model.predict(obs_vec, deterministic=False) # Use observation part
